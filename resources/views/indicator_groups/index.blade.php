...
@section('content')
    <div class="container mx-auto px-4 py-6 bg-white p-6 rounded-lg shadow-lg" style="margin-top: 10px;">
        ...
        {{-- <form method="GET" action="{{ route('tasks.index') }}" class="">
            <div class="mb-6 flex flex-wrap gap-4 mb-4">
                ...
                <a class="fa fa-filter" style="margin-top: 45px;cursor: pointer;" onclick="window.location.href='{{ route('tasks.index') }}'"></a>
        </div>
    ...
    <div class="mb-6 flex justify-end gap-4 mb-4">
        <a href="{{ route('indicator_groups.create') }}" class="inline-block bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 transition duration-300"> Thêm mới</i></a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-lg">
                ...
                <tbody>
                    @foreach ($taskGroups as $index => $item)
                        <tr class="border-b border-gray-200">
                            <td class="py-3 border border-gray-300 px-6 text-center">{{ $index + $taskGroups->firstItem() }}</td>
                            ...
                            <td class="py-3 border border-gray-300 px-6 text-center">
                                <button class="bg-yellow-300 text-white px-4 py-2 rounded-lg shadow hover:bg-yellow-600 transition duration-300 ml-2"
                                    onclick="window.location.href='{{ route('indicator_groups.edit', $item) }}'">
                                    <i class="fas fa-edit"></i> 
                                </button>
                            </td>
                            <td class="py-3 border border-gray-300 px-6 text-center">
                                <form id="delete-form-{{ $item->id }}" action="{{ route('indicator_groups.destroy', $item) }}" method="POST" style="display:inline;" onsubmit="confirmBeforeDelete({ event })">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
