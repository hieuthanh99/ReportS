...
@section('content')
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        @endif
        <form action="{{ route('organization_types.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-lg" id="document-form">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="type_name" name="type_name" class="form-input w-full border border-gray-300 rounded-lg p-2 resize-none" rows="4" required>{{ old('type_name') }}</textarea>
                    @error('type_name')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mt-4 flex" style="justify-content: space-between">
                <a href="{{ route('organization_types.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg shadow-lg hover:bg-gray-700 transition duration-300 mr-2">Quay lại</a>
